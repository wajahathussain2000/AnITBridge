...
const HeroSection = ({ flying = false, contactStyle = false, title = 'We Bridge IT Gaps', subtitle = 'Empowering your business with innovative IT solutions, cloud, networking, and digital transformation. Let us take your technology to the next level.' }) => {
  ...
  useEffect(() => {
    if (flying && containerRef.current) {
      ...
      // Define safe zones (right side and bottom areas)
      const safeZones = [
        // Right side zone
        {
          x: Math.max(rect.width * 0.6, contentAreaWidth + 50),
          y: Math.random() * (rect.height - robotHeight),
          width: rect.width - Math.max(rect.width * 0.6, contentAreaWidth + 50) - robotWidth,
          height: rect.height - robotHeight
        },
        // Bottom zone (below content)
        {
          x: Math.random() * (rect.width - robotWidth),
          y: Math.max(rect.height * 0.7, contentAreaHeight + 50),
          width: rect.width - robotWidth,
          height: rect.height - Math.max(rect.height * 0.7, contentAreaHeight + 50) - robotHeight
        }
      ];
    ...
    if (flying && !hasFlown) {
      ...
      setTimeout(() => {
        ...
        if (rect) {
          ...
          const safeY = Math.random() * (rect.height - robotHeight);
